  ...
  return (
    <div className={`grid grid-cols-1 gap-3 md:gap-4 lg:gap-6 md:grid-cols-2 lg:grid-cols-3 ${className}`}>
      ...
      <InsightCard
        ...
        type={
          sinistrosTrend?.direction === 'up' ? 'warning' : sinistrosTrend?.direction === 'down' ? 'success' : 'neutral'
        }
        badge={
          sinistrosTrend
            ? {
      ...
      >
        <div className='space-y-3'>
          {sinistrosTrend ? (
            <>
              <div className='flex items-center justify-between'>
                ...
                <span className='font-medium text-foreground'>
                  {sinistrosTrend.change > 0 ? '+' : ''}
                  {sinistrosTrend.change} sinistros
                </span>
              </div>

              <div className='flex items-center justify-between'>
                <span className='text-sm text-muted-foreground'>Média diária:</span>
                <span className='font-medium text-foreground'>{averages.avgSinistros}</span>
              </div>
            ...
            <div className='text-sm text-muted-foreground'>Dados insuficientes para análise de tendência</div>
          )}
        </div>
      </InsightCard>
      ...
      >
        <div className='space-y-3'>
          {statusInsights ? (
            <>
              <div className='space-y-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <span className='font-medium text-foreground'>{statusInsights.pendingPercent.toFixed(1)}%</span>
                </div>

                <div className='flex items-center justify-between'>
                  <span className='text-sm text-muted-foreground'>Em análise:</span>
                  <span className='font-medium text-foreground'>{statusInsights.analysisPercent.toFixed(1)}%</span>
                </div>

                <div className='flex items-center justify-between'>
                  <span className='text-sm text-muted-foreground'>Rejeitados:</span>
                  <span className='font-medium text-foreground'>{statusInsights.rejectedPercent.toFixed(1)}%</span>
                </div>
              </div>
            ...
            <div className='text-sm text-muted-foreground'>Nenhum dado de status disponível</div>
          )}
        </div>
      </InsightCard>
