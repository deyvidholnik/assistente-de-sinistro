...
export function MetricsCards({ metrics, isDark }: MetricsCardsProps) {
  ...
  const calculateTrends = () => {
    ...
    const sinistrosTrend =
      sinistrosPorDia.length >= 2
        ? ((sinistrosPorDia[sinistrosPorDia.length - 1].count - sinistrosPorDia[sinistrosPorDia.length - 2].count) /
            (sinistrosPorDia[sinistrosPorDia.length - 2].count || 1)) *
          100
        : 0
  ...
  const kpiData = [
    ...
    },
    {
      ...
      icon: Activity,
      color:
        trends.engajamento > 80
          ? 'hsl(var(--status-success))'
          : trends.engajamento > 60
        ...
        return (
          ...
          >
            <CardContent className='p-2 sm:p-3 md:p-4 text-center min-w-0'>
              ...
              >
                <IconComponent className='w-3 sm:w-4 md:w-5 lg:w-6 h-3 sm:h-4 md:h-5 lg:h-6 text-foreground' />
              </div>
              ...
              <div className='text-xs text-muted-foreground mb-1 truncate leading-tight'>{kpi.title}</div>
