...
export function StepFotos() {
  const { currentFotoStep, fotoStepFiles, handleFotoStepUpload, removeFotoStepFile, isProcessingOCR, tipoSinistro } =
    useForm()
  ...
  const pequenosReparosSteps = [
    ...
    },
    {
      ...
      posicionamento: ['Aproxime-se a 20-30cm do chassi', 'Use flash se necessário', 'Mantenha o celular estável'],
  ...
  const currentStepData =
    tipoSinistro === 'pequenos_reparos' ? pequenosReparosSteps[currentFotoStep] : fotoVeiculoSteps[currentFotoStep]
  ...
  return (
    <div className='space-y-4 sm:space-y-6'>
      ...
      <div
        className={`${
          tipoSinistro === 'pequenos_reparos' ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'
        } rounded-lg p-4 sm:p-6 border mx-2 sm:mx-0`}
      >
        <h3
          className={`font-semibold ${
            tipoSinistro === 'pequenos_reparos' ? 'text-blue-800' : 'text-orange-800'
          ...
          <div className='space-y-1 sm:space-y-2'>
            ...
            <ul
              className={`space-y-1 text-xs sm:text-sm ${
                tipoSinistro === 'pequenos_reparos' ? 'text-blue-700' : 'text-orange-700'
              }`}
            >
              {currentStepData.instrucoes.map((instrucao, index) => (
                ...
                >
                  <span
                    className={`${
                      tipoSinistro === 'pequenos_reparos' ? 'text-blue-600' : 'text-orange-600'
        ...
        <div
          className={`mt-3 sm:mt-4 p-2 sm:p-3 bg-white rounded border ${
            tipoSinistro === 'pequenos_reparos' ? 'border-blue-200' : 'border-orange-200'
          }`}
        >
          <p
      ...
      <div className='flex justify-center'>
        ...
        >
          ...
          <div className='text-center'>
            ...
            <div className='text-xs opacity-90'>{hasFile ? 'Delete para trocar' : 'Abrir Câmera'}</div>
          </div>
        </Button>
      </div>
